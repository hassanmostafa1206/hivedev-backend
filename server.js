...
const NOTIFY_EMAIL  = process.env.NOTIFY_EMAIL  || 'hivedev.pro@gmail.com';
const RESEND_API_KEY = process.env.RESEND_API_KEY || 're_VgMNp1zB_PwBTNxtmbWSmaGNQzJQGvjK6';
...
async function sendNotification(name, email, type, message) {
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'HiveDev <onboarding@resend.dev>',
        to: NOTIFY_EMAIL,
